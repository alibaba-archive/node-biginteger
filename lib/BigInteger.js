...
exports.fromBuffer = function (signum, magnitude) {
  ...
  _bigInteger._mag = _bigInteger._stripLeadingZeroBytes(magnitude);

  if (signum < -1 || signum > 1)
      throw new Error("Invalid signum value");

  if (_bigInteger._mag.length==0) {
      _bigInteger._signum = 0;
  } else {
      if (signum == 0)
          throw new Error("signum-magnitude mismatch");
      _bigInteger._signum = signum;
  }
};
...
var _fromLong = exports.fromLong = function (val) {
  ...
  if (val < 0) {
    val = -val;
    _bigInteger._signum = -1;
  } else {
    _bigInteger._signum = 1;
  }

  if (val.high === 0) {
    _bigInteger._mag = new Array(1);
    _bigInteger._mag[0] = val.low;
  } else {
    _bigInteger._mag = new Array(1);
    _bigInteger._mag[0] = val.high;
    _bigInteger._mag[1] = val.low;
  }
  return _bigInteger;
};
...
exports.fromString = function (val, radix) {
  ...
  if (cursor === len) {
    _bigInteger._signum = 0;
    _bigInteger._mag = new Buffer([0]);
    return;
  }
  numDigits = len - cursor;
  _bigInteger._signum = sign;
  ...
  _bigInteger._mag = _bigInteger._trustedStripLeadingZeroInts(magnitude);
  return _bigInteger;
};
...
BigInteger.prototype.bitLength = function () {
  ...
  if (n == -1) { // bitLength not initialized yet
    ...
    if (len == 0) {
      ...
      var magBitLength = ((len - 1) << 5) + this._bitLengthForInt(this._mag[0]);
       if (this._signum < 0) {
           // Check if magnitude is a power of two
           var pow2 = (this._bitCount(this._mag[0]) == 1);
...
BigInteger.prototype._bitLengthForInt = function (i) {
...
BigInteger.prototype.abs = function () {
  return this._signum >= 0 ? this : this.negative();
};
...
BigInteger.prototype.negative = function () {
  ...
  return n._fromMag(this._mag, -this._signum);
};

BigInteger.prototype._fromMag = function (magnitude, signum) {
...
BigInteger.prototype._getInt = function (n) {
  ...
  var magInt = this._mag[this._mag.length - n - 1];

  return (this._signum >= 0 ? magInt :
...
BigInteger.prototype.toString = function (radix) {
  ...
  var maxNumDigitGroups = (4 * this._mag.length + 6) / 7;
  // String
  ...
  while (tmp._signum != 0) {
    ...
    var a = new MutableBigInteger(tmp._mag),
    var b = new MutableBigInteger(d._mag);
    var r = a.divide(b, q);
  //     BigIntegerTest q2 = q.toBigIntegerTest(tmp.signum * d.signum);
  //     BigIntegerTest r2 = r.toBigIntegerTest(tmp.signum * d.signum);
