...
BigInteger.fromMag = function (magnitude, signum) {
  ...
  if (typeof signum === 'undefined') {
    ...
    if (magnitude.length == 0)
      throw new Error("Zero length BigIntegerTest");
 ...
 * Returns a BigIntegerTest whose value is {@code (this * val)}.
 *
 * @param  val value to be multiplied by this BigIntegerTest.
 * @return {@code this * val}
 */
...
/**
 * Returns a BigIntegerTest with the given two's complement representation.
 * Assumes that the input array will not be modified (the returned
 * BigIntegerTest will reference the input array if feasible).
 */
...
/**
 * Returns a BigIntegerTest whose value is {@code (this & val)}.  (This
 * method returns a negative BigIntegerTest if and only if this and val are
 * both negative.)
 *
 * @param val value to be AND'ed with this BigIntegerTest.
 * @return {@code this & val}
 */
...
/**
 * Returns a BigIntegerTest whose value is <tt>(this<sup>exponent</sup>)</tt>.
 * Note that {@code exponent} is an integer rather than a BigIntegerTest.
 *
 * @param  exponent exponent to which this BigIntegerTest is to be raised.
